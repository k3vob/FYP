...
inputMatrix = np.array([
    df.loc[df['id'] == ID, [feature for feature in features]].as_matrix()
    for ID in IDs
])
# Shape: (1424, ?, 1) = (numIDs, numIDTimestamps, y)
labelMatrix = np.array(
    [df.loc[df['id'] == ID, ['y']].as_matrix() for ID in IDs])
...
def generateBatch(IDPointer, TSPointer, isTraining=True):
    ...
    for ID_ix in range(IDPointer, IDPointer + batchSize):
        for i, TS in enumerate(
                range(TSPointer, TSPointer + Constants.sequenceLength - 1)):
            if TS not in ID_TS_dict[IDs[ID_ix]] and (
                    TS + 1) in ID_TS_dict[IDs[ID_ix]]:
    ...
    for i, ID_ix in enumerate(range(IDPointer, IDPointer + batchSize)):
        ...
        for j, TS in enumerate(
                range(TSPointer, TSPointer + Constants.sequenceLength)):
    ...
    epochComplete = False
    if IDsComplete and TSsComplete:  # if all IDs & timestamps are processed
...
def generateTestBatch():
    batchSize, inputs, labels, lengths, resetState, _, _, _ = generateBatch(
        0, 0, isTraining=False)
    return batchSize, inputs, labels, lengths, resetState
