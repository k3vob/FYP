...
x = tf.placeholder(tf.float32,
                   [None, Constants.sequenceLength, DataWorker.numFeatures])
y = tf.placeholder(tf.float32, [None, Constants.sequenceLength, 1])
xTensors = tf.unstack(
    x, axis=1)  # [seqLength tensors of shape (batchSize, numFeatures)]
yTensors = tf.unstack(y, axis=1)  # seqLen, ?, 1

lengths = tf.placeholder(tf.float32, [None])
masks = tf.cast(
    tf.cast(tf.range(Constants.sequenceLength), tf.float32) < tf.reshape(
        lengths, (-1, 1)), tf.float32)
...
layers = [
    tf.contrib.rnn.BasicLSTMCell(
        Constants.numHidden,
        forget_bias=Constants.forgetBias,
        state_is_tuple=True) for _ in range(Constants.numLayers)
]
...
accuracy = tf.reduce_sum(
    accuracy, axis=0)  # ?, 1 -> sum of accuracies per batch
accuracy = tf.reshape(accuracy, [-1])  # ?, 1 -> ?
accuracy = tf.divide(accuracy, tf.maximum(
    lengths, 1))  # ?, 1 -> average accuracy per batch
accuracy = tf.reduce_mean(accuracy)  # average over all batches
accuracy = 1 - accuracy

optimiser = tf.train.GradientDescentOptimizer(Constants.learningRate).minimize(
...
with tf.Session() as session:
    ...
    for i in range(1):
        batchSz, batchX, batchY, batchLengths, _, \
            IDPointer, TSPointer, epochComplete = \
            DataWorker.generateBatch(IDPointer, IDPointer)
        dict = {
            batchSize: batchSz,
            x: batchX,
            y: batchY,
            lengths: batchLengths
        }
    ...
    testAccuracy = session.run(accuracy, {
        batchSize: testSize,
        x: testX,
        y: testY
    })
