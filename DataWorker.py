...
def getTradingDates(start=Constants.startDate, end=Constants.endDate):
    """Get all dates in the range that the NYSE was trading on."""

    print("\nRETRIEVING TRADING DATES...\n")
    # Days NYSE was open
    days = pandas_market_calendars.get_calendar('NYSE').valid_days(start_date=start, end_date=end)
    # Just save date (YYYY-MM-DD)
    dates = [day.date() for day in days]
    dates.remove(Constants.missingDate)

    pk.dump(dates, open(Constants.dataDir + "tradingDates.p", "wb"))
    return dates
...
def getSP500Index(tradingDates):
    ...
    SP500Index = SP500Index.loc[str(tradingDates[0]): str(tradingDates[-1])]
    ...
    for i in range(len(tradingDates)):
        ...
        dates.append(date)
    SP500Index.set_index([dates], inplace=True)

    pk.dump(SP500Index, open(Constants.dataDir + "SP500Index.p", "wb"))
    return SP500Index


def getDailyMarketData(tradingDates, tickers_dict):
    ...
    for i, ticker in enumerate(tickers_dict.keys()):
        ...
        tickerData = quandl.get_table(
            'WIKI/PRICES',
            ticker=ticker,
            date={'gte': tradingDates[0],
                  'lte': tradingDates[-1]},
            paginate=True
        )

        # If any dates missing, skip
        if tickerData.shape[0] != len(tradingDates):
            print("\tDates Missing")
...
def downloadSentimentFiles(tickers_dict):
    ...
    for ticker, data in tickers_dict.items():
        ...
        # if file already downloaded and extracted
        if csvFile in os.listdir('.'):
...
def getSentimentData(tradingDates, tickers):
    ...
    for i, ticker in enumerate(tickers):
        ...
        for line in sentimentData[1:]:
            ...
            if dateTime.date() < tradingDates[0] - dt.timedelta(days=1):
                continue
            if (dateTime.date() == tradingDates[0] - dt.timedelta(days=1)
            ...
            while dateTime.date() not in date_sentimentScores:
                dateTime = dateTime + dt.timedelta(days=1)
                if dateTime.date() > tradingDates[-1]:
                    endReached = True
                    break
...
def addLabels(data, tickers):
    ...
    print("\nAdding Labels...\n")
    data['down'] = (data['adj_close'].pct_change(Constants.predictionWindow) < 0).astype(float)
    data['up'] = (data['adj_close'].pct_change(Constants.predictionWindow) >= 0).astype(float)

    # data['down3'] = (data['adj_close'].pct_change(Constants.predictionWindow)
    #                  <= -Constants.labelThreshold).astype(float)
    #
    # data['down'] = ((data['adj_close'].pct_change(Constants.predictionWindow) > -Constants.labelThreshold) &
    #                 (data['adj_close'].pct_change(Constants.predictionWindow) < 0)).astype(float)
    #
    # data['up'] = ((data['adj_close'].pct_change(Constants.predictionWindow) >= 0) &
    #               (data['adj_close'].pct_change(Constants.predictionWindow) < Constants.labelThreshold)).astype(float)
    #
    # data['up3'] = (data['adj_close'].pct_change(Constants.predictionWindow)
    #                >= Constants.labelThreshold).astype(float)

    # Drop nulls vs drop first n per ticker ?????
...
tradingDates = pk.load(open(Constants.dataDir + "tradingDates.p", "rb"))
...
print("Trading Dates:\t\t", tradingDates[0], "-", tradingDates[-1])
...
numTotalDates = data.loc[tickers[0]].shape[0]
numTrainingDates = int(numTotalDates * Constants.trainingPercentage)
...
# tickers_dict = getTickers_Names(tradingDates)
# dailyMarketData, tickers_dict = getDailyMarketData(tradingDates, tickers_dict)
# tickers = list(tickers_dict.keys())
# SP500Index = getSP500Index(tradingDates, tickers)
# tickers_dict = getSentimentURLs(tickers_dict)
# downloadSentimentFiles(tickers_dict)
# sentimentScores = getSentimentData(tradingDates, tickers)
